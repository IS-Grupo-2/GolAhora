...
export default function ReservaModal({ reserva, onClose, onSave }) {
    ...
    const handleSubmit = (e) => {
        ...
        // Buscamos el cliente seleccionado para extraer sus datos reales
        const clienteSel = clientes.find(c => c.idUsuario === Number(form.clienteId) || c.id === Number(form.clienteId));
                    ...
                    <form id="form-reserva" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        ...
                        <div className="form-row">
                            <div className="form-group">
                                ...
                                <input name="fechaUso" type="date" value={form.fechaUso} onChange={handleChange} required />
                            </div>
                            <div className="form-group">
                                <label>Monto Total Calculado ($)</label>
                                <div style={{ background: '#f1f5f9', padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--border)', fontWeight: 'bold', color: 'var(--purple)' }}>
